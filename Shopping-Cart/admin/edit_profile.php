...
if (isset($_POST['submit']) && !empty($_POST['submit']) && !empty($id)) {
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (!empty($mobile)) {
			$mobileStr = strval($mobile);
			if (strlen($mobileStr) != 9) {
		...
		if (
			...
			if ($_FILES["photo"]["error"] > 0) {
				...
				$rnd = rand(100, 999);
				$rnd = $rnd . "_";
				$fileName = $rnd . trim($_FILES['photo']['name']);
				$tmpName  = $_FILES['photo']['tmp_name'];
				$fileSize = $_FILES['photo']['size'];
				$fileType = $_FILES['photo']['type'];
				$target = "Images/";
				$target = $target . $rnd . basename($_FILES['photo']['name']);
				$photo = $rnd . basename($_FILES['photo']['name']);
...
<div class="registration__container">
		...
		<form id="myForm" onsubmit="return validateForm()" action="" method="POST" enctype="multipart/form-data">
			...
			<input class="registration__input" type="submit" value="Update" name="submit">
...
if (isset($_POST['submit'])) {
    if (empty($nameErr) && empty($lnameErr) && empty($usernameErr) && empty($emailErr) && empty($mobileErr) & empty($emailErr) && empty($pswErr) && empty($confirm_passwordErr) && empty($photoErr)) {
        ...
        if (empty($photo)) {
            $fileQuery = '';
            if (!empty($image)) {
                $fileQuery = ", `photo` = '" . $image . "'";
            }
        } else {
            $fileQuery = ", `photo` = '" . $photo . "'";
        }

        $sql = "UPDATE users SET `first_name` = '$first_name', `last_name` = '$last_name', `email` = '$email', '$photo =' 'photo'" . $fileQuery . " WHERE id= " . $id;
        echo $sql;
